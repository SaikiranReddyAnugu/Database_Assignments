1.select * from film f inner join film_category fc on f.film_id=fc.film_id
 ...
 2.select f.*,count(c.customer_id) as coustmer_count from customer c inner join
 rental r on c.customer_id=r.customer_id 
 inner join inventory i on r.inventory_id=i.inventory_id 
 inner join film f on f.film_id=i.film_id
 inner join film_category fc on f.film_id=fc.film_id  
 inner join category ca on ca.category_id=fc.category_id 
 and ca.name="horror" group by f.film_id order by coustmer_count desc limit 3;
...
select  distinct c.* from customer c inner join address a on a.address_id=c.address_id
 ...
 inner join inventory i on r.inventory_id=i.inventory_id inner join film f on f.film_id=i.film_id
 inner join film_actor fa on f.film_id=fa.film_id inner join actor ac on fa.actor_id=ac.actor_id
